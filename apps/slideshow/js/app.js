(function($) {
	...
	window.Controller = Backbone.Model.extend({
		...
		next : function() {
			var firstTile = this.get('firstTile'), tiles = this.get('tiles'), tilePosition = [];
		...
		previous : function() {
			var lastTile = this.get('lastTile'), tiles = this.get('tiles'), tilePosition = [], length = window.tiles.length - 1;
		...
		closeDetails: function(){
		  ...
		  $('#slideController').removeClass('detail');
		},
	...
	$(document).ready(function() {
		...
		DetailsView = Backbone.View.extend({
			...
			events:{
			  'click #slideController #backButton': 'closePanel'
			},
